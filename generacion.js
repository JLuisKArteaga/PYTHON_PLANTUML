...
class PythonToPlantUML {
    constructor() {
        ...
        // Clases del sistema Python a ignorar como clases en el diagrama
    ...
    inicializarEventos() {
        ...
        this.btnVerOnline = document.getElementById('btnVerOnline');
        ...
        this.btnVerOnline.addEventListener('click', () => this.verEnPlantUMLOnline());
        this.btnRespaldar.addEventListener('click', () => this.respaldarCodigo());
        
        this.ventanaEditar.addEventListener('input', () => this.debouncePreview());
    ...
    debouncePreview() {
        ...
        this.debounceTimer = setTimeout(() => this.actualizarVistaPrevia(), 1000);
    }
    ...
    procesarCodigo() {
        ...
        try {
            ...
            this.btnVerOnline.disabled = false;
    ...
    procesarHerenciaYRealizacion(clase) {
        clase.herencia.forEach(padre => {
            // Ignorar object y clases del sistema para herencia simple
            if (padre === 'object') return;
            
            const esClaseSistema = this.clasesSistema.has(padre);
            const esInterfaz = this.esInterfaz(padre) || 
                              clase.decoradores.some(d => d.includes('abstract')) ||
                              padre === 'ABC';
            
            if (esInterfaz && this.opciones.realizacion.checked) {
                // Realizacion: no mostrar ABC como clase, solo la relacion
                const nombreInterfaz = esClaseSistema ? (padre === 'ABC' ? 'Drawable' : padre) : padre;
                
                // Si es ABC, usar el nombre de la clase como interfaz implicita
                if (padre === 'ABC') {
                    // La clase actual implementa una interfaz abstracta
                    this.relaciones.realizacion.push({
                        linea: `${clase.nombre} ..|> ${clase.nombre}Interface : <<implement>>`
                    });
                } else {
                    this.relaciones.realizacion.push({
                        linea: `${clase.nombre} ..|> ${nombreInterfaz} : <<implement>>`
                    });
                }
                this.contadorRelaciones.realizacion++;
            } else if (this.opciones.herencia.checked && !esClaseSistema) {
                // Herencia normal solo si no es clase del sistema
    ...
    esInterfaz(nombreClase) {
        const interfacesComunes = ['Protocol', 'Callable', 'Iterable', 'Iterator', 
                                   'Mapping', 'Sequence', 'Set', 'Container', 'Sized',
                                   'Hashable', 'Awaitable', 'Coroutine', 'AsyncIterable'];
        return interfacesComunes.includes(nombreClase) || 
               nombreClase.startsWith('I') || 
               nombreClase.endsWith('Interface') ||
               nombreClase.endsWith('Protocol');
    }
    ...
    verEnPlantUMLOnline() {
        ...
        if (!codigo.trim()) {
            alert('No hay codigo para visualizar');
            return;
        }

        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'http://www.plantuml.com/plantuml/uml/';
        form.target = '_blank';
        ...
        this.mostrarNotificacion('Abriendo en PlantUML...', 'success');
    }
    ...
    actualizarVistaPrevia() {
        ...
        const hint = this.previewSection.querySelector('.preview-hint');
        hint.innerHTML = 'Usa "Ver en PlantUML" para ver el diagrama<br>' +
                        '<small>O copia el codigo en <a href="http://www.plantuml.com/plantuml" target="_blank">plantuml.com</a></small>';
    }
