 ...
 * Detecta patrones GOF (fase de conocimiento)
 */
...
class PythonToPlantUML {
    ...
    detectarFactoryMethod(clase, codigo) {
        ...
        const parametrosTipo = codigo.match(/def\s+\w+\s*\([^)]*:\s*(\w+)\s*\)/);
        const tipoRetorno = codigo.match(/->\s*(\w+)\s*:/);
    ...
    generarPlantUML() {
        ...
        // Seccion de patrones GOF (fase de conocimiento)
