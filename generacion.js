...
class PythonToPlantUML {
    ...
    analizarRelaciones(lineas) {
        ...
        for (let i = 0; i < lineas.length; i++) {
            ...
            if (matchClase) {
                claseActual = this.clases.get(matchClase[1]);
    ...
    procesarHerenciaYRealizacion(clase) {
        clase.herencia.forEach(padre => {
            if (padre === 'object' || padre.includes('metaclass') || padre.includes('Meta')) return;
    ...
    esInterfaz(nombreClase) {
        const interfacesComunes = ['ABC', 'Protocol', 'Callable', 'Iterable', 'Iterator', 
    ...
    procesarAtributo(nombre, valor, clase) {
        ...
        if (this.opciones.composicion.checked) {
            ...
            if (matchNew) {
                ...
                if (this.clases.has(tipo) && tipo !== clase.nombre) {
                    // Sintaxis correcta: Todo *-- Parte : nombre
            ...
            if (esColeccion) {
                ...
                if (matchTipo) {
                    ...
                    if (this.clases.has(tipo)) {
                        // Sintaxis correcta: Todo o-- Parte : nombre
